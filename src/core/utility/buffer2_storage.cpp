...
/*! MMap File Storage
 */
class Buffer2Storage : public IndexStorage {
 public:
  ...
  class Segment : public IndexStorage::Segment,
   ...
   public:
    ...
    //! Constructor
    Segment(Buffer2Storage *owner, IndexMapping::Segment *segment)
        : segment_(segment),
          owner_(owner),
          capacity_(static_cast<size_t>(segment->meta()->data_size +
                                        segment->meta()->padding_size)) {
      const int offset = segment_->meta()->data_index + owner_->get_context_offset();
      zvec::ailego::BufferID buffer_id =
          zvec::ailego::BufferID::VectorID(owner->file_name_, offset, static_cast<int>(capacity_));
      ctx_ = ailego::BufferManager::Instance().acquire_context(buffer_id);
    }
    ...
    size_t read(size_t offset, const void **data, size_t len) override {
      ...
      ailego::BufferHandle::Pointer buffer_handle = std::make_unique<ailego::BufferHandle>(ctx_.get());
      *data = (char *)buffer_handle->pin_vector_data() + offset;
      return len;
    }
    ...
    size_t read(size_t offset, MemoryBlock &data, size_t len) override {
      ...
      ailego::BufferHandle::Pointer buffer_handle = std::make_unique<ailego::BufferHandle>(ctx_.get());
      data.reset(buffer_handle, offset);
      if (data.data()) {
        return len;
      } else {
        LOG_ERROR(
   ...
   private:
    ...
    Buffer2Storage *owner_{nullptr};
    size_t capacity_{};
    ailego::BufferManager::BufferContext::Pointer ctx_{nullptr};
  };

  //! Destructor
  virtual ~Buffer2Storage(void) {
    this->cleanup();
  }
  ...
  IndexStorage::Segment::Pointer get(const std::string &id, int) override {
    ...
    if (!segment) {
      return Buffer2Storage::Segment::Pointer();
    }
    return std::make_shared<Buffer2Storage::Segment>(this, segment);
  }
...
INDEX_FACTORY_REGISTER_STORAGE_ALIAS(BufferStorage, Buffer2Storage);
