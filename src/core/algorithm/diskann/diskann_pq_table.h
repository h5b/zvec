...
class PQTable {
 ...
 public:
  ...
  void aggregate_coords(const uint32_t id_num, const diskann_id_t *ids,
                        const uint8_t *all_coords, const uint64_t dim,
                        uint8_t *out);

  void pq_dist_lookup(const uint8_t *pq_ids, const size_t n_pts,
                      const size_t pq_nchunks, const float *pq_dist_buffer,
                      float *dists);

  void compute_dists(const uint32_t id_num, const diskann_id_t *ids,
